...
def images():
    ...
    print(
        f"The image has {img_shp[0]} rows, {img_shp[1]} columns, and {img_shp[2]} channels (R,G,B)"
    )
    print()
    plt.imshow(img)
    plt.xlabel("x")
    ...
    print(
        f"Question B12: shape of rock_canyon.jpg image converted to grayscale\n{img_gray_shp}"
    )
    print(
        f"The image has {img_gray_shp[0]} rows and {img_gray_shp[1]} columns. No third component, since gray is one channel."
    )
    print()
    ...
    plt.plot(
        np.mean(np.mean(img, axis=2), axis=0), "-k", linewidth=2.0, label="mean RGB"
    )
    ...
    plt.plot(
        np.mean(np.mean(img, axis=2), axis=1), range(0, img_shp[0]), "-k", linewidth=2.0
    )
